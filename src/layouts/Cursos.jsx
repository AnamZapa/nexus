...
export default function Cursos() {
  ...
  const abrirEditar = (curso) => { setCursoSeleccionado(curso); setForm({ ...curso }); setError(""); setModal("editar"); };
  const abrirEliminar = (curso) => { setCursoSeleccionado(curso); setModal("eliminar"); };
  const cerrarModal = () => { setModal(null); setCursoSeleccionado(null); setForm(FORM_VACIO); setError(""); };
  const handleChange = (e) => { const { name, value } = e.target; setForm(prev => ({ ...prev, [name]: value })); };

  const validar = () => {
    if (!form.nombre || !form.categoria || !form.duracionHoras || !form.cuposMaximos || !form.precio || !form.instructor) {
  ...
  const guardarCurso = () => {
    ...
    if (modal === "crear") {
      setCursos(prev => [...prev, { ...form, id: Date.now(), cuposDisponibles: cuposNum, cuposMaximos: cuposNum, duracionHoras: parseInt(form.duracionHoras), precio: parseFloat(form.precio), mensajeDisponibilidad: disp }]);
    } else {
      setCursos(prev => prev.map(c => c.id === cursoSeleccionado.id ? { ...c, ...form, duracionHoras: parseInt(form.duracionHoras), precio: parseFloat(form.precio), cuposMaximos: cuposNum, mensajeDisponibilidad: disp } : c));
    }
    cerrarModal();
  };

  const eliminarCurso = () => { setCursos(prev => prev.filter(c => c.id !== cursoSeleccionado.id)); cerrarModal(); };
    ...
    <AdminLayout>
      <div className="postulantes-content">
        ...
        <div className="top-section">
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
            ...
            <div className="search-box">
              <FaSearch />
              <input type="text" placeholder="Buscar por nombre, categoría o instructor..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
            </div>
          </div>
        ...
        <div className="table-container">
          {filtrados.length === 0 ? (
            <p style={{ textAlign: "center", padding: "40px", color: "#64748b" }}>No se encontraron cursos.</p>
          ) : (
            <table className="postulantes-table">
              <thead>
                <tr>
                  <th>ID</th><th>Nombre</th><th>Categoría</th><th>Duración (h)</th>
                  <th>Cupos</th><th>Precio</th><th>Instructor</th><th>Disponibilidad</th><th>Acciones</th>
                </tr>
              </thead>
              <tbody>
                {filtrados.map(c => (
                  <tr key={c.id}>
                    ...
                    <td>{c.instructor}</td>
                    <td>
                      <span className={`estado ${c.mensajeDisponibilidad === "AGOTADO" ? "rechazado" : c.mensajeDisponibilidad?.includes("últimos") ? "pendiente" : "aprobado"}`}>
                        {c.mensajeDisponibilidad}
                      </span>
                    </td>
                    <td>
                      <div className="acciones">
                        <button className="btn-view" title="Editar" onClick={() => abrirEditar(c)}><FaEdit /></button>
                        <button className="btn-reject" title="Eliminar" onClick={() => abrirEliminar(c)}><FaTrash /></button>
                      </div>
                    </td>
                  </tr>
                ))}
